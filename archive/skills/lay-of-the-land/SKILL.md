...
description: Pre-workflow reconnaissance — a sourced map of what currently exists in an unfamiliar area of the codebase, before a requirement is written/refined or a workflow (feature-dev, refactor, bugfix) starts. Every finding carries a confidence score and a concrete source; assumptions are escalated as open questions, never presented as facts. Produces {topic}_lay-of-the-land.md. Use ad-hoc / pre-refinement, or as the Phase 0 recon that feeds integration-analysis, refactor-audit, bug-investigation, or a refinement discussion. The body of /lay-of-the-land (formerly /trigger-discovery-phase).
---
...
> **Litmus test:** if a developer can copy-paste your output and start _building_, you have gone too deep — that is `integration-analysis` / `refactor-audit` / a techspec, not recon. Recon tells them _what is there and where_, leaving them ready to write or refine the requirement. If a line states what _should_ be built or changed, delete it.
...
- **Pre-workflow Phase 0**: before `integration-analysis` / `refactor-audit` / `bug-investigation` — so those phases start from facts, not guesses.
- **Ad-hoc mid-session**: a brief "how does X actually work here?" with no doc — the lightweight path (see Process).

## When NOT to use

- You already know the area and need the _feature-integration_ map → `integration-analysis`. The _refactor_ map → `refactor-audit`. A _bug's_ root cause → `bug-investigation`. Recon is upstream of all three.
...
- **Recommended Next Steps** — which downstream workflow / skill (`/triage` if unsure · `integration-analysis` · `refactor-audit` · `bug-investigation`); what to focus first; stakeholders to consult.
