...
- **No mandate handed to you (default — you're on the main thread):** you're the *coordinator*.
  ...
  > **Downshift check (before launching workers).** The 3-way parade earns its ~3× token cost only when the approaches would land in *materially different places*. Run **fewer workers (1–2) or the single-approach sibling (`pragmatic-techspec`)** when any of these holds: an upstream decision is already **locked** (an approved proposal / ADR / memory fixes the schema, the mapping, or the approach — the three drafts will converge by construction); the integration analysis already supplies a **file-level implementation map + an enumerated test list** (the workers would re-derive the same inventory three times); or the change is **small / a closed-decision polish** (a handful of files, no open architectural fork). Say you're downshifting and why; reserve the full 3-way for genuinely open architectural latitude.
...
3. **Patterns & conventions found** — a table; every row points to real code you read. `| Pattern | Source (file:line) | Usage here |`. Every citation **verified, not guessed — and anchored on a stable token** (the symbol name / a unique literal / nearest heading), with the line number a *hint that drifts*: a sibling task inserting lines above shifts every bare `:NN` below it. Write `AppContext.tsx → useAuth() (≈:55)`, not `AppContext.tsx:55`. When you **re-read a citation you (or an upstream doc) wrote earlier, re-locate it by the anchor and re-verify it against current source** before relying on it — never reuse an inherited `file:line` verbatim (branches move; symbols get renamed).
4. **Component design** — each component: file path, responsibilities, dependencies, interfaces.
5. **Implementation map** — file-by-file. For each touched file: path + change type (Create/Modify), what changes and **why** (one sentence), before/after snippets for non-trivial edits (no placeholder `[NewThing]` — use the real names), and which pattern from §3 it follows. Cite touched locations by stable anchor + line-hint (per §3), so the map survives the inserts later tasks make. Verification extends to **test-file locations** (one Glob per named test file/project — a techspec that guessed the wrong test project survived two reviewers) and to **lifted code's closure**: when a step says "lift/copy X verbatim", enumerate X's symbol dependencies (usings/imports, helpers, config keys) and diff the whole source file per environment for incidental hunks — verbatim lifts carried compile-breaking references twice. **Config keys** introduced by the spec follow the repo's nesting convention, describe their value, and split secret-vs-static — a spike's poor key shape otherwise rides through every downstream artifact.
