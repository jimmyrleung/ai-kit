...
Resolve references relative to this skill folder, then consult
[provider capabilities](references/shared/provider-capabilities.md) for tool availability,
model configuration, literal-input probes, and runtime recovery. These are host
capabilities, not properties inferred from a model name.
...
4. **Complete charter coverage.** Track each required scope as pending, running, returned, verified, or explicitly excluded with owner-authorized scope/reason. If the sole security or domain worker stalls, other workers cannot supply that coverage by agreement. Check its actual live handle and durable output; an observation timeout alone is not termination. Resume or reassign the missing scope using available host recovery, preserving completed deliverables. Do not declare complete until every required charter is verified. Any proposed exclusion changing the user's scope needs authorization. Record unresolved contradictions and distinguish source inspection from runtime evidence.
5. **Verify in tiers.** Worker self-reports are claims, not results: scripted structural pass over **all** outputs (counts, required sections, link/citation resolution) + full reads of the top-risk few (>10 outputs → tiered, not line-by-line). Dedup findings by `file:line:category` — exact-key dedup silently killed a distinct co-located finding — and pass `also_flagged_by` into any verify prompt.
