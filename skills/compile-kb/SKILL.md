---
name: compile-kb
description: "Synthesizes accumulated source notes into a separate, rebuildable wiki and a dated “what changed” digest for a non-Hermes Markdown KB vault. Use to compile, consolidate, roll up, extract trends, initialize, convert, or migrate a KB / wiki / vault in any domain. Excludes Hermes-managed KBs, single-source ingestion, and KB queries; populated-vault conversion requires an approved plan."
...
Run every invocation, before anything else. Follow [authorized work](references/shared/authorized-work.md):
existing authorization covers its stated scope; a generic confirmation cannot precede the read-only
work/no-op check. Resolve bundled references from this skill folder.
...
2a. **Conversion resume precedes initialized/no-op detection.** Inspect any existing
   `init-proposal.md` conversion manifest and its approval boundary. Reconcile its approved
   rows against current bytes/status under the resume rules below, even when SCHEMA and wiki
   scaffolding already exist. Resume approved nonterminal or reopened rows through Convert
   before ordinary compilation; shared scaffolding never proves the remaining origins were
   processed. An unresolved blocked row keeps conversion visibly pending. If approval cannot
   be established, do not relocate anything or infer it from the manifest's existence; resolve
   that question only for dependent conversion work. Report pending conversion separately from
   any independently authorized compile result. A terminal, verified manifest permits the
   ordinary initialized/no-op path without rewriting it.
...
5. **`SCHEMA.md` or the `wiki/` tree missing → not initialized.** Classify, then branch — **never *silently* restructure** (conversion runs only behind an approved plan):
   ...
   - **Populated but unstructured** (many `.md`, no SCHEMA/3-layer — e.g. a course dump): this is a **migration, not an init**, run in **two approval-gated steps — propose, then (on approval) convert + synthesize.** **Step 1 — propose & PAUSE:** produce **a complete disposition inventory** accounting for **100%** of the vault, a draft root `SCHEMA.md`, and the per-file conversion manifest defined below; write it to `init-proposal.md` at the vault root and **wait for approval — do not relocate anything yet.** **Step 2 — convert on approval:** resume that manifest for the approved domains, relocate source bytes into root `raw/<domain>/`, create `sources/<domain>/` notes, then run the generic synthesis head (plus any built enhancements). Preserve each origin's Git tracking intent and never stage or commit as a side effect. **One canonical layout per vault:** root = `SCHEMA.md` + `raw/` + `sources/` + `wiki/`, archetypes as domains all compiling into one shared `wiki/`. **Classify inputs, don't dump them (rule 10):** known-ingestable files are converted; exhaust stays put; first-party lesson/sample code is explored as a study source rather than migrated file-by-file. **Never silently delete, and never 100%-tag a folder as exhaust** — rescue first-class sources individually; an unexpected extension pauses for confirmation. For a domain fitting no known archetype, propose the archetype/update and still synthesize generically. **Ask before converting 10+ domains or moving large folders** (rule 8).
...
## Write sets and resumable conversion state

Resolve the operation before writing; one run may execute several modes in this order:

| Mode | Permitted writes |
|---|---|
| compile | `wiki/` pages/navigation/log/digest; frontmatter-only `sources/` status transitions |
| seed | new or evidence-grounded refreshed `sources/` notes and their explicit raw-ownership records; `raw/` remains read-only |
| convert | approved `SCHEMA.md`/scaffold, `init-proposal.md` manifest updates, approved relocations into `raw/`, new `sources/` notes, then compile writes |

Before conversion, `init-proposal.md` contains one stable manifest row per source/exhaust/unknown
file: `item_id`, origin, planned destination or `leave`, classification, domain/archetype, original
Git tracking state, intended tracking state, content identity, source-note path, status, and note.
Statuses are `proposed`, `raw-ready`, `source-ready`, `compiled`, `excluded`, or `blocked`.

Update one row only after verifying that step's destination bytes/references. On resume, verify every
non-proposed row against current paths and content identity before skipping it; an existing `raw/`
directory never proves its remaining rows were converted. A missing/mismatched artifact reopens that
row or marks it blocked. If the origin is absent but the planned destination has the recorded bytes,
resume at `raw-ready` instead of relocating again. If both paths exist or identities disagree, stop
that row for owner review. This makes interruption resumable without duplicate notes or skipped files.

Preservation-first tracking policy: content that was tracked at its origin has
`intended_tracking: tracked` at its raw destination; untracked input remains untracked unless the
approved plan explicitly changes that choice. Relocate bytes without staging. Git may later detect
the tracked origin/destination pair as a rename when the user explicitly approves staging. Never
run an index-mutating move, stage, or commit as part of conversion itself.

Before relocation, enumerate links/references in each input. After each row reaches `raw-ready`,
verify its content identity and that source notes/manifests retain the origin and current raw path;
after synthesis, check affected wiki links. Record intentional reference rewrites per row.
Advance a source note to `status: integrated` only after its target wiki writes and checks succeed.
Append a completed convert log/digest only when every approved manifest row is terminal
(`compiled`, `excluded`, or explicitly accepted `blocked`); an interrupted run remains visibly in
...
- Cluster candidate sources by `domains`/`themes` + content. For each cluster meeting the SCHEMA page-creation threshold (central to one source, or appears across 2+), create or update the target **knowledge page**:
  - **Cross-source synthesis is what ingest alone never produces** — roll the cluster into whichever page type fits the material, from Karpathy's **archetype-agnostic vocabulary**: `topics/` (cross-source trend/overview), `concepts/` (one idea), `comparisons/`, plus per-source `summaries` and `entity`/overview pages where useful. Place under the vault's `wiki/` tree per SCHEMA `compile_targets` — the regeneratable Compile layer; synthesis never changes raw bytes or source-note bodies. The orchestrator may apply the explicitly permitted source-status transition after checks succeed. **A single-source cluster gets a faithful overview/summary page — never a fabricated cross-source "trend."**
  ...
  - Cross-link (≥2 `[[wikilinks]]`), update each page's `## Source Trail`, and return the consumed source notes for the orchestrator's post-check `status: integrated` transition.
...
- `git diff --check`; inspect tracked, staged, unstaged, and untracked state. For compile/seed,
  `raw/` is outside the write set. For conversion, reconcile every raw origin/destination against
  the approved manifest and tracking intent; any staged conversion path must come from a separate
  explicit user staging action, never from the conversion operation. Media/secrets remain excluded
  unless the approved vault policy names a safe tracked source.
...
- **Show the complete operation-specific Git state to the user.** For compile/seed, propose only the
  declared curated writes. For conversion, the proposal includes the tracked origin/destination
  pairs whose manifest says `intended_tracking: tracked`, plus approved schema/source/wiki/manifest
  files; it leaves originally untracked raw inputs untracked unless separately approved. Never
  stage or commit implicitly. With explicit approval, stage exactly the reviewed paths and then ask
  separately before commit unless the workflow already supplied that authority.
...
Conversion is **not** a compile head — it's the one-time, approval-gated relocation that gets a populated vault into the canonical `raw/`+`sources/`+`wiki/` layout so the generic head can synthesize it. Already-canonical vaults skip it only when preflight finds no pending approved conversion rows; an interrupted manifest takes precedence over the presence of scaffolding.

### Convert (approval-gated, idempotent)
- Run only after the step-5 disposition proposal is **approved**. Process approved manifest rows,
  not whole-directory presence. For each real source, relocate bytes to `raw/<domain>/` under the
  preservation-first tracking policy and create one `sources/<domain>/` note (short summary +
  frontmatter: `status: summarized`, `domains`, `themes`, `date_consumed`, `source_type`,
  `original_filename`, `raw:`, `sha256_prefix`). Grouped notes track the primary raw file in
  frontmatter and every secondary hash/path in `## Source Trail`.
...
- **Idempotent/resumable:** verify each manifest row's status, paths, bytes, references, and source
  note before skipping it. Existing `raw/<domain>/` content is reconciled row-by-row; it never
  suppresses the rest of a partial migration. Honor rules 3, 6, and 8.
...
6. **No implicit staging or commit.** Show the complete Git state. Compile/seed keep `raw/`
   read-only. Conversion preserves each input's tracking intent and includes tracked relocations in
   the reviewable commit set only after explicit staging approval; originally untracked raw stays
   untracked unless separately approved. Never commit secrets, and apply the vault's explicit media
   policy.
...
## Output
Within `$vault_path`: knowledge pages under the shared `wiki/` tree, refreshed navigation/log, and
the compilation digest. The Write sets section is authoritative: compile writes wiki output plus
source status transitions; seed writes source notes/ownership records while raw stays read-only;
approved conversion also writes schema/scaffold/manifest state and relocates declared inputs into
raw before seed+compile. Conversion preserves per-file tracking intent. Every mode shows its exact
write set; none stages or commits implicitly.
