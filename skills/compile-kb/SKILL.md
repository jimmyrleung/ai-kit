---
name: compile-kb
description: "Compile a markdown knowledge-base vault — synthesize accumulated source notes into cross-source trend/topic pages, refresh `index.md` + `_meta` maps, adversarially review changed pages, and emit a dated 'what changed' compilation digest (incremental + idempotent). Use when asked to compile, consolidate, synthesize, or roll up a KB / brain-kb / wiki / vault, for a weekly or mid-week compilation, to extract trends across accumulated notes, or for a 'what's new since last time' digest. Detects an uninitialized vault and guides setup first. Synthesis-wiki archetype (v0). Invoke as /compile-kb."
arguments: vault_path
---

# compile-kb — the "Compile" in Collect → Compile → Wiki

You are a KB compiler. You run a periodic, **incremental** synthesis pass that turns a vault's accumulated `sources/` into cross-source knowledge pages and a "what changed" digest. You do **not** ingest individual sources, answer queries, or run the full health-check lint — and you **never** silently restructure a populated vault.
...
2. Check for `SCHEMA.md`, `index.md`, `log.md` at the vault root.
3. **All present → orient** (the existing-wiki path): read `SCHEMA.md` in full, read `index.md`, read the last ~30 lines of `log.md`. Resolve the archetype from the SCHEMA **Compile** block (or a `## Archetype` line); if absent, **infer** — `sources/` + typed `concepts/`+`topics/` folders ⇒ `synthesis-wiki` — record the inference and suggest the user add an explicit `archetype:`. If the archetype is **not** `synthesis-wiki`, STOP: name the archetype, say its compile head isn't built yet, and hand off. Otherwise continue to Process.
4. **SCHEMA/index/log missing → not initialized.** Classify, then branch — **never auto-restructure**:
   - **Greenfield** (≈0 content `.md` outside `raw/`, `templates/`, `.obsidian/`): offer a **guided init** — ask the archetype + domain (be specific), write `SCHEMA.md` (with a Compile block), `index.md`, `log.md`, and the synthesis-wiki folder scaffold; confirm; suggest first sources. Then stop (nothing to compile yet) unless sources already exist.
   - **Populated but unstructured** (many `.md`, no SCHEMA/3-layer — e.g. a course dump): this is a **migration, not an init**. Do **not** move or rewrite anything. Produce a *proposal*: sample the content, infer the archetype + a draft `SCHEMA.md` + a proposed `raw/ ↔ sources/ ↔ wiki` file mapping for a representative slice; present it for the user to approve/edit; STOP. In a headless run, write the proposal to `_meta/init-proposal.md` and stop. The heavy migration is a separate guided step (`kb-init`).
   - **Partial** (SCHEMA present, index/log missing): repair the missing scaffolding from current files — do not clobber the SCHEMA.
...
- Find the last compile: the most recent `## [YYYY-MM-DD] compile |` entry in `log.md` (or "never").
...
- Cluster candidate sources by `domains`/`themes` + content. For each cluster meeting the SCHEMA page-creation threshold (central to one source, or appears across 2+), create or update the target **knowledge page**:
  - **Trend/topic pages are the primary v0 output** — the cross-source synthesis that ingest alone never produces (e.g. rolling many digests into a trend page). Place per the vault's SCHEMA (`topics/`, `concepts/`, `_meta/`).
...
- Patch `index.md`: add new pages under the correct type section; **recompute** the `> Last updated: … | Total pages: N` header (fix drift).
- Update affected `_meta/*-map.md` MOCs.
- Append `## [YYYY-MM-DD] compile | <one-line summary>` to `log.md`; rotate to `log-YYYY.md` if it exceeds 500 entries.

### Phase 5 — Emit the compilation digest
Write the dated digest the user reads and points harnesses at, to `<digest_dir>/YYYY-MM-DD-compile.md` (default `_meta/compilations/`). Shape:
...
compile_targets: [topics, concepts, _meta]   # folders this head may write
digest_dir: _meta/compilations
```

Defaults when the block is absent: `archetype: synthesis-wiki` (only if inferable, else STOP), `cadence: weekly`, `midweek_threshold: 15`, targets `[topics, concepts, _meta]`, digest `_meta/compilations`. Recommend the user add an explicit block; do not silently edit their SCHEMA without showing the change.

## Important rules
1. **Orient before acting** — read SCHEMA + index + recent log every session. Skipping it duplicates pages and misses cross-references.
...
## Output
Within `$vault_path`: knowledge pages under their SCHEMA-defined type folders; `index.md` + `_meta` maps refreshed; a `log.md` `compile` entry appended; the compilation digest under `<digest_dir>/`. Never writes `raw/`. Commits only curated files, only with approval.
