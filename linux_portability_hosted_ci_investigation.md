...
1. **VERIFIED** — fixture apply operations call `SyncEngine.execute_action`, which reaches `create_link()` at `scripts/sync-skills.py:387` for absent managed entries.
...
2. **VERIFIED** — `invoke_hook()` at `scripts/sync-skills.py:805` launched the path directly.
...
1. **VERIFIED** — `test_compatibility_posix_wrappers_forward_and_reject_private_home_override` at `tests/test_sync_skills.py:720` executes Bash wrappers.
...
2. **Integration — script execution:** Windows cannot execute a `.py` hook as a native executable.
   - Falsifier: a hosted traceback still reports WinError 193 when the hook command begins with `sys.executable`.
3. **Configuration — macOS path alias:** fixtures treated lexical and canonical temporary paths as interchangeable when constructing relative symlinks.
   - Falsifier: resolved fixture paths still produce `/private/Users/...` or direct `/var` versus `/private/var` assertion failures.
4. **Test scope — adapter mismatch:** a POSIX execution test ran on Windows despite dedicated PowerShell coverage.
   - Falsifier: the Windows-specific adapter or junction coverage is skipped after the POSIX fixture is gated.

## Proposed solution

- In `scripts/sync-skills.py:387`, pass the `cmd.exe`, `mklink`, option, link, and target tokens as separate subprocess arguments.
- In `scripts/sync-skills.py:805`, prefix `.py` action hooks with `sys.executable`.
...
- In `tests/test_sync_skills.py:720`, run the POSIX-wrapper fixture only when `os.name != "nt"`.
...
- Side effects: `.py` action hooks now explicitly use the active interpreter; non-Python hooks keep direct execution. Junction type, ownership evidence, transaction format, preserve policy, and live-home boundaries are unchanged.
