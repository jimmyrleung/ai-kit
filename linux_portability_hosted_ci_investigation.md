...
1. **VERIFIED** — fixture apply operations call `SyncEngine.execute_action`, which reaches `create_link()` at `scripts/sync-skills.py:405` for absent managed entries.
...
2. **VERIFIED** — `invoke_hook()` at `scripts/sync-skills.py:823` launched the path directly.
...
### Windows dangling-junction safety

1. **VERIFIED** — run `33544478522` passed Ubuntu/macOS and reduced Windows to one failure: `test_unowned_dangling_neighbor_blocks_orphan_action` did not raise `PlanConflict`.
2. **VERIFIED** — `check_unowned_extra_links()` blocks only an unowned link whose `resolved_target` is inside the canonical skill tree.
3. **VERIFIED** — `resolved_link_target()` used `realpath()` on the live link path. For the dangling Windows junction, the missing target was not recoverable from that traversal, so the safety prefix check did not match.
4. **VERIFIED** — resolving the stored raw target relative to the link parent preserves the intended target even when it is missing; normalization then handles Windows namespace representations.
...
3. **Safety classification — dangling Windows junction:** resolving through a dangling live junction loses its stored target and lets an unowned canonical-tree neighbor escape the fail-closed check.
   - Falsifier: the hosted dangling-neighbor fixture still completes prune instead of raising `PlanConflict` when resolution starts from the raw junction target.
4. **Integration — script execution:** Windows cannot execute a `.py` hook as a native executable.
   - Falsifier: a hosted traceback still reports WinError 193 when the hook command begins with `sys.executable`.
5. **Configuration — macOS path alias:** fixtures treated lexical and canonical temporary paths as interchangeable when constructing relative symlinks.
   - Falsifier: resolved fixture paths still produce `/private/Users/...` or direct `/var` versus `/private/var` assertion failures.
6. **Test scope — adapter mismatch:** a POSIX execution test ran on Windows despite dedicated PowerShell coverage.
