...
.DESCRIPTION
  ...
  What it does (NOTHING touches the canonical ai-kit tree; Claude is provably unaffected):
    ...
    3. Orchestr. : Codex has no command primitive (prompts deprecated). The ~25
                   thin per-phase shims need nothing (their skill is already
                   junctioned). The 5 family ORCHESTRATORS + 3 per-task EXECUTORS
                   carry real wiring (phase sequencing, S/M/L/XL classifier,
                   gates, Workflow 1/2/3) no skill owns -> each is GENERATED as a
                   Codex skill (implicit-invocation:false; a multi-phase workflow
                   must never auto-trigger). Canonical commands/ is untouched
                   (Claude keeps the /x UX at zero context cost).
...
# --- 3. Orchestrator/executor commands -> generated Codex skills ----------
...
# These 8 carry real orchestration prose no skill owns. Generated Codex-only,
# implicit-invocation OFF (a 5-phase workflow must never auto-trigger).
$OrchCmds = @(
...
foreach ($cn in $OrchCmds) {
  ...
  if ((Test-Path $dest) -and -not (Test-Path (Join-Path $dest $GenMarker))) {
    Add-Result $cn 'orch' 'skip' 'name exists and is not kit-generated — left untouched'
    continue
  }
  if ($WhatIf) { Add-Result $cn 'orch' 'would-gen' "Codex skill <- commands/$cn.md"; continue }
  ...
  $oy = @(
    ...
    "  default_prompt: `"Run the `$$cn workflow; it sequences the kit skills per its phases.`""
    "policy:"
    "  allow_implicit_invocation: false"
  ) -join "`n"
  Set-Content -LiteralPath (Join-Path $dest 'agents\openai.yaml') -Value ($oy + "`n") -NoNewline -Encoding utf8
  Set-Content -LiteralPath (Join-Path $dest $GenMarker) -Value "source: commands/$cn.md`n" -NoNewline -Encoding utf8
  Add-Result $cn 'orch' 'generated' 'Codex orchestrator skill (implicit-invocation off)'
}
...
Write-Host ("skills+agents+orchestrators exposed: {0}   issues: {1}" -f $results.Count, $bad.Count)
if ($bad.Count -gt 0) {
