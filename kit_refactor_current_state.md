...
- **Ownership family** (backlog 27, restore-vs-strip open): `adr-first`, `challenge-me`, `debug-first`, `onboard-me`, `predict-first`, `record-decision`. Note the live coupling is real: `close/SKILL.md:45-49,165` references `/record-decision` and `/adr-first`; `walkthrough-implementation/SKILL.md:13-14,29-30,74` routes to `onboard-me`/`challenge-me`. Until you decide, those are dead routes in two live skills.
